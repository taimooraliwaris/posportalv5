...
export function ProductTile({
  ...
  const status = available <= 0 ? "out" : available <= reorderPoint ? "low" : "healthy";
  ...
  return (
    <button
      ...
      onClick={() => {
        if (status === "out") {
          toast.warning(`${product.name} is currently out of stock (qty 0), but added to cart`);
        }
        onAdd();
      }}
      className={cn(
    ...
    >
      <div className={cn("flex flex-1 items-center justify-center", toneClass[product.tone ?? "amber"] || toneClass["amber"])}>
        <ProductIcon name={product.icon || "Package"} className="h-12 w-12 opacity-80" />
      </div>
