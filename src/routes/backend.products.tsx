                      ...
                      {items.map((p) => (
                        <tr key={p.id} className="hover:bg-muted/30 transition-colors">
                          ...
                          <td className="px-4 py-3 align-top">
                            ...
                            {p.name_ur && (
                              <div
                                className="text-xs text-muted-foreground mt-0.5 font-urdu"
                                dir="rtl"
                              >
                                {p.name_ur}
                              </div>
                          ...
                          <td className="px-4 py-3 align-top text-xs text-muted-foreground">
                            {selectedCategory === "spare_parts" ? (
                              <div className="flex flex-wrap gap-1">
                                {p.specs?.model && (
                                  <span className="bg-accent/10 border border-accent/20 text-accent px-1.5 py-0.5 rounded">
                                    {p.specs.model}
                                  </span>
                                )}
                                {p.specs?.position && (
                                  <span className="bg-primary/10 border border-primary/20 text-primary px-1.5 py-0.5 rounded">
                                    {p.specs.position}
                                  </span>
                                )}
                                {p.specs?.variant && (
                                  <span className="bg-muted border border-border text-foreground px-1.5 py-0.5 rounded">
                                    {p.specs.variant}
                                  </span>
                                )}
                              </div>
                              ...
                              <div className="flex flex-col gap-0.5">
                                ...
                                <div>
                                  {p.brand} {p.specs?.ply ? `• ${p.specs.ply}` : ""}{" "}
                                  {p.specs?.tread ? `• ${p.specs.tread}` : ""}
                                </div>
                              </div>
                            )}
                          </td>
