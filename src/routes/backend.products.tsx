...
function ProductsPage() {
  ...
  const search = Route.useSearch() as { category?: string };
  const selectedCategory = search.category || "spare_parts";
  ...
  const visibleProducts = productList.filter((p) => {
    ...
    if (selectedFamily) {
      ...
      if (selectedCategory === "tyres" || selectedCategory === "tubes") {
        return (
          p.specs?.["size"] === selectedFamily ||
          p.specs?.["position"] === selectedFamily ||
          p.primary_model_code === selectedFamily
        );
      }
    }
  ...
  const spareFamilies = Array.from(
    new Set(
      productList
        .filter((p) => p.category === "spare_parts" && p.specs?.["family"])
        .map((p) => p.specs.family),
    ),
  );
  const tyreVehicles = Array.from(
    new Set(
      productList
        .filter((p) => p.category === "tyres" && p.primary_model_code)
        .map((p) => p.primary_model_code),
    ),
  );
    ...
    <BackendLayout title="Products">
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex gap-2 items-center">
          <Button onClick={() => setIsAddOpen(true)} className="h-8 px-4 text-xs">
            + Add product
          </Button>
        </div>
      </div>
            ...
            {Object.entries(groupedProducts).map(([group, items]) => (
              <div key={group}>
                <div className="flex items-center gap-1.5 px-3 py-2 text-[11px] font-medium text-muted-foreground border-b border-border bg-muted/30">
                  ...
                  {group}{" "}
                  <span className="text-[10px] bg-background px-1.5 rounded-full ml-1">
                    {items.length} items
                  </span>
                </div>
                ...
                {items.map((p) =>
                  selectedCategory === "spare_parts" ? (
                    <div
                      key={p.id}
                      className="grid grid-cols-[90px_1fr_70px_50px_60px_40px_30px] items-center px-3 py-1.5 border-b border-border text-[11px] gap-1.5 hover:bg-muted/10 transition-colors"
                    >
                      ...
                      <div>
                        ...
                        {p.name_ur && (
                          <div className="text-[10px] text-muted-foreground text-right dir-rtl font-urdu">
                            {p.name_ur}
                          </div>
                        )}
                      </div>
                      <span className="text-[10px] text-muted-foreground">
                        {p.primary_model_code || "—"}
                      </span>
                      {p.specs?.["position"] ? (
                        <span className="bg-accent/20 text-accent text-[10px] px-1.5 rounded-sm w-fit">
                          {p.specs.position}
                        </span>
                      ) : (
                        <span>—</span>
                      ...
                      <input
                        className={cn(
                          "bg-accent/10 border-none outline-none font-mono font-medium rounded-sm w-8 px-1",
                          p.stock_qty <= 0 ? "text-destructive" : "text-success",
                        )}
                        value={p.stock_qty}
                        onChange={(e) =>
                          updateProductInCatalog(p.id, { stock_qty: Number(e.target.value) || 0 })
                        }
                      />
                    ...
                    <div key={p.id} className="inline-block w-1/3 p-1.5 align-top">
                      <div className="border border-border rounded-md p-2">
                        ...
                        <div className="text-[10px] text-muted-foreground mt-[1px] uppercase">
                          {p.brand}
                        </div>
                        <div className="text-[10px] text-muted-foreground/70">
                          {p.specs?.["ply"]} · {p.specs?.["tread"]}
                        </div>
